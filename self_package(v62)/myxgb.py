...
class myxgb:
    ...
    def dmultisoftmaxloss(self,X,y,y_hat):
        """Return the first-order derivative of the softmax loss
        """
        n,k=y.shape
        fm=np.exp(y_hat)#(n,k)
        norm=np.sum(np.exp(y_hat),axis=1)#(n,)
    ...
    def ddmultisoftmaxloss(self,X,y,y_hat):
        """Return the second-order derivative of the softmax loss
        """
        n,k=y.shape
        fm=np.exp(y_hat)#(n,k)
        norm=np.sum(np.exp(y_hat),axis=1)#(n,)
...
#                print('at iteration{} evluation acc is {}'.format(t+1,accuracy_score(y_evl,y_pred_label)))
                #training
                fm=np.exp(y_hat)#(n,k)
                norm=np.sum(np.exp(y_hat),axis=1)#(n,)
                p=fm/np.tile(norm.reshape(-1,1),k)#(n,k)
                y_pred_label=np.argmax(p,axis=1)
                y_training_label=np.argmax(y_tmp,axis=1)
                print('at iteration{} evluation acc is {}'.format(t+1,accuracy_score(y_training_label,y_pred_label)))
        ...
        for t in range(0,self.n_estimators):
            ...
            if self.loss_function == "reg:squarederror":
                    ...
                    print('at iteration{} evluation rmse is {}'.format(t+1,np.sqrt(mean_squared_error(y_pred,y_evl))))
                    print('at iteration{} training rmse is {}'.format(t+1,np.sqrt(mean_squared_error(y_hat,y_tmp))))
                    iteration_validation.append(np.sqrt(mean_squared_error(y_pred,y_evl)))
            else:
                    print('at iteration{} evluation acc is {}'.format(t+1,accuracy_score(y_evl,expit(y_pred)>0.5*1)))
                    print('at iteration{} training acc is {}'.format(t+1,accuracy_score(y_tmp,expit(y_hat)>0.5*1)))
                    iteration_validation.append(accuracy_score(y_evl,expit(y_pred)>0.5*1))
            g = self.dloss_func(X_tmp,y_tmp,y_hat)
            h = self.ddloss_func(X_tmp,y_tmp,y_hat)
           ...
           #w_root,obj_root=tree_root.get_weight(None,g,h,self.lbda,new_X=X[indices,:])# since this is the root of a tree it doesn't have any parents
            ...
            # pruning
            if self.prune:
                num_pruning = self.prune_tree(tree)
                if num_pruning > 0:
                    tree_obj = tree.objective(self.gamma)
                    if self.verbose > 0:
                        print("{} nodes pruned, ".format(num_pruning))
            ...
            if tree_obj > 0. or self.trees[-1].is_leaf():
                ...
                break
        return self,iteration_validation
    
    def _predict(self, X,y_true):
        """Make predictions.used in the fit() function
        """
